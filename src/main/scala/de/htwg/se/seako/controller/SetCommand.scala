...
class SetCommand (row:Int, col: Int, cell: Cell, controller: Controller, grid: Grid[Cell]) extends Command{
  ...
  override def undoStep(): Unit = {
    var oldCell = oldgrid.cell(row, col)
    tmpCell = controller.grid.cell(row, col)
    tmpCell match {
      case oldCell.players. => controller.grid.replaceCell(row,col, oldCell(row, col))
    }
  //  controller.grid = controller.grid.replaceCell(row,col, oldgrid.cell(row, col)
  }
