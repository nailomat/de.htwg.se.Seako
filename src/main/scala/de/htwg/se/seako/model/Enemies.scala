...
case class Enemies(zombies: List[Zombie],mutants: List[Mutant],bosses: List[Boss]) {
  ...
  def getValue: Integer = {
    var value : Integer = 0
    Enemies match {
      case zombies.size > 0 => zombies.foreach(Zombie => value = value + Zombie.getAttack)
      case mutants.size > 0 => mutants.foreach(Mutant => value = value + Mutant.getAttack)
      case bosses.size > 0 => bosses.foreach(Boss => value = value + Boss.getAttack)
    }
    value
  }
